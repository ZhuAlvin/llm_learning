...
> 📊 **项目状态**：33/33 notebooks 已完成 (100%) | 平均质量评分 93.7/100（M1-M9）+ M10 执行验证通过（内容评分待统一审计）⭐⭐⭐⭐⭐
...
8. ✅ **概念先定义后使用**：任何概念/术语在被多次引用前，必须在首次出现处给出正式定义（见下方规范）
9. ✅ **问题驱动叙事**：优先回答”为什么学、解决什么问题、如何落地”
10. ✅ **统一业务主线**：模块说明需映射到真实场景。课程采用**双主线并行**设计：主线1「电商客服智能助理」（文本/NLP, M1-M9）、主线2「通用多模态平台（**便利店货架监控示范**）」（视觉/多模态, M10 及 M05/M07/M08 穿插点）
11. ✅ **读者导向交付**：每个模块必须定义可交付产出和最低完成标准
...
#### 规则：概念/术语在被多次引用前，必须在首次出现处给出正式定义

当���个概念、术语或问题作为关键示例在文档或 Notebook 中被**反复引用**时，必须在首次出现的位置给出**正式的定义或说明**，确保读者在第一次遇到它时就能理解其含义。

#### 什么算"正式定义"

根据概念类型，定义可以包含以下要素中的一项或多项：

| 概念类型 | 定义要素 | 示例 |
|---------|---------|------|
| 逻辑/数学概念 | 文字说明 + 真值表/公式 + 直觉解释 | XOR：真值表 + "两输入不同时输出 1" + 散点图 |
| 技术术语 | 一句话定义 + 适用场景 | 梯度累积："将多个小 batch 的梯度求和后再更新参数" |
| 问题/现象 | 问题描述 + 产生条件 + 直觉类比 | 梯度消失："深层网络中梯度逐层衰减至极小，浅层参数几乎无法更新" |

#### 反例与正例

**反例**（已修正）：
- ❌ 代码注释写 `Solve XOR problem`、FAQ 提到"XOR 不是线性可分"、思考题答案引用"无法解决 XOR"，但从未正式解释 XOR 是什么
- ❌ 多处提到"梯度消失"现象，但直到最后才给出定义

**正例**（当前标准）：
- ✅ 在 §3.3 "为什么需要多层"首次引入 XOR 时，给出完整定义（文字说明 + 真值表 + ASCII 散点图 + 线性不可分的几何直觉），后续所有引用不再重复定义
- ✅ 在首次提到"梯度消失"时，用一句话给出定义，后续章节可直接使用

#### 适用范围

1. **Notebook 文档**：每个 `.ipynb` 文件独立检查；概念在文件内首次出现时定义
2. **README / LEARNING_GUIDE**：同上，每个文件独立检查
3. **跨文件不继承**：即使在前一个 Notebook 中已定义，新 Notebook 首次使用时仍需提供至少一句话的简要说明（可配合"回顾 Module X.Y"指向原始定义）

#### 例外情况

以下情况可以不给出定义：
- 本文件的前置知识列表中已明确要求读者掌握的概念（如"前置知识：Module 1.3 反向传播"）
- 极其通用的概念：函数、变量、循环、数组等编程基础
- 已在同一 cell 的上下文中自然解释的概念

#### 检查清单

创建或修改内容时，检查：

- [ ] 是否存在被多次引用但从未正式定义的概念或术语？
- [ ] 关键示例（如 XOR、梯度消失等贯穿性概念）是否在首次出现处有完整定义？
- [ ] 跨文件引用的概念是否至少有一句话简要说明或回顾指向？
...
1. 📚 本章概览 (Overview)
   - 学习目标（3-5 个明确目标）
   - 核心问题（要解决的关键问题）
   - 🏢 业务场景（NLP 模块锚定到电商客服智能助理，多模态模块锚定到通用多模态平台（**便利店货架监控示范**），2-4 句）
   - 知识地图（本章在整体课程中的位置）
   - 预计学习时间
...
- [ ] 是否明确映射统一业务主线（NLP 主线：电商客服智能助理 / 多模态主线：通用多模态平台（便利店货架监控示范））？
- [ ] 是否包含”可交付产出”（2-3 个）？
- [ ] 是否包含”学习曲线设计”与”每章建议阅读顺序”？
- [ ] 每个 notebook 描述是否包含至少 1 条业务问题映射？
- [ ] 每个 notebook 概览中是否包含 `🏢 业务场景` 子节（位于核心问题之后、知识地图之前）？
- [ ] 每条学习路径是否包含”最低完成标准”？

### ✅ 概念完整性（2026-03-02 新增）

- [ ] 是否存在被多次引用但从未正式定义的概念或术语？
- [ ] 关键示例/贯穿性概念是否在首次出现处有完整定义？
- [ ] 跨文件引用的概念是否有简要说明或回顾指向？
...
2. 映射统一业务主线（NLP 主线：电商客服智能助理 / 多模态主线：通用多模态平台（便利店货架监控示范））
3. 确定学习目标（3-6 个）
4. 定义可交付产出与最低完成标准
5. 列出核心概念与实践项目
6. 估算学习时间与学习曲线分段
...
- 统一业务主线映射（NLP 主线：电商客服智能助理 / 多模态主线：通用多模态平台（便利店货架监控示范））
...
每个模块应包含 `LEARNING_GUIDE.md`，用于说明“如何学完本模块并完成验收”，不再采用“质量审计报告”叙事。
...
- [ ] 是否映射统一业务主线？
- [ ] 是否提供统一学习动作链路？
- [ ] 每个 notebook 是否包含“问题/示例/最低标准/产出物”？
- [ ] 是否至少提供 2 条学习路径（含推荐路径）？
- [ ] 是否明确学习曲线分段与衔接？
- [ ] 是否给出 3-5 条可检查验收标准？
- [ ] 是否列出常见卡点与应对？
- [ ] 是否说明与下一模块的能力衔接？
...
**步骤：**
1. 参考最新 LEARNING_GUIDE（2026-03-02 后版本）
2. 使用 [学习指南文档规范](#学习指南文档规范) 模板
3. 以“问题驱动”写模块价值和业务映射
4. 为每个 notebook 补全“问题/示例/最低标准/产出物”
5. 提供 2-3 条学习路径并标注推荐路径
6. 明确模块验收标准和与下一模块衔接
7. 运行质量检查清单
...
- ✅ 9/9 模块 README 补充业务问题映射和术语注释
- ✅ 质量检查清单新增 notebook 业务场景子节检查项
...
**项目状态**：33/33 notebooks 已完成 | 平均质量评分 93.7/100 (M1-M9) ⭐⭐⭐⭐⭐
