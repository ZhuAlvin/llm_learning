...
- 掌握量化技术（动态量化、静态量化、PTQ (Post-Training Quantization, 训练后量化)、QAT (Quantization-Aware Training, 量化感知训练)）
- 实现模型剪枝（非结构化、结构化、幅度剪枝）
- 理解知识蒸馏的原理和实现方法
- 掌握 ONNX (Open Neural Network Exchange, 开放神经网络交换格式) 模型导出和图优化技术
- 了解硬件加速方案（TensorRT、OpenVINO、CoreML）
- 构建高性能模型服务系统
- 掌握 MLOps (Machine Learning Operations, 机器学习运维) 生产最佳实践
...
`SLO (Service Level Objective, 服务水平目标) -> 技术方案 -> 最小验证 -> 压测结果 -> 风险控制 -> 运维清单`
...
**业务问题映射**：
- “客服高峰期响应延迟超 SLA (Service Level Agreement, 服务水平协议)？” -> 量化 + 剪枝降低推理延迟
- “压缩模型后回复质量明显变差？” -> 知识蒸馏保精度 + 校准策略
- 🎥 “边缘设备（Jetson/树莓派）能跑多模态模型吗？” -> INT8 量化 + ONNX（M07 技术在 M10 便利店货架案例的边缘盒子部署中扩展为 TensorRT + INT8 PTQ，详见 Module 10.4）
...
- 动态批处理：提升吞吐量 5-10x
- 缓存优化：LRU (Least Recently Used, 最近最少使用) 缓存，90%+ 延迟减少
- 负载均衡和自动扩展策略
- 监控和可观测性：Prometheus 指标
- 容器化和 Kubernetes 部署
- CI/CD (Continuous Integration/Continuous Deployment, 持续集成/持续部署) 流水线和部署策略
- 业务映射：用“促销期间并发激增”理解服务扩容与限流
**业务问题映射**：
- “双 11 咨询量翻 5 倍怎么抗住？” -> 动态批处理 + 缓存 + 自动扩容
- “新模型上线后效果反而更差？” -> 金丝雀部署 + 自动回滚机制
...
- 监控与漂移检测：KS (Kolmogorov-Smirnov) 检验、PSI (Population Stability Index, 群体稳定性指数)、告警系统
- 错误处理与调试：输入验证、优雅降级
- SLA/SLO/SLI (Service Level Agreement/Objective/Indicator, 服务水平协议/目标/指标) 监控与性能优化
- 事故响应与复盘：事故管理、Postmortem模板
- 业务映射：用“线上错误回复事故”演练告警、回滚与复盘
**业务问题映射**：
- “线上模型偶尔给出不安全回复？” -> 输出过滤 + 告警 + 事故复盘流程
- “运行半年后效果慢慢变差？” -> 数据漂移检测 + 自动重训练触发
