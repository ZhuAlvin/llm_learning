...
**主线 2（多模态/CV）：`通用多模态平台`** 🆕 — M05/M07/M08 含代码实践 + Module 10 综合实战

- M05 实践：LoRA-for-ViT 代码示例（同一技术跨模态验证）→ 指向 Module 10.4
- M07 实践：边缘部署 benchmark（PyTorch/ONNX/TensorRT/CoreML 对比）→ 指向 Module 10.4
- M08 实践：CLIP + FAISS 图文混合检索代码 → 指向 Module 10.2
- Module 10：安防监控/零售分析/医疗影像的全链路实战（选型→训练→微调→边缘部署+合规）
