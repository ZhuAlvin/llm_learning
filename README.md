...
这是一套完整的大语言模型学习指南，涵盖从基础数学到前沿研究的全部内容。通过 **33 个精心设计的 Jupyter Notebooks**（含 Module 10 多模态扩展），你将系统掌握 LLM 的理论基础、工程实践和实际应用。

### 🎯 核心特色

- ✅ **理论与实践结合**：每个概念都配有代码实现和可视化
- ✅ **循序渐进**：从 NumPy 到 PyTorch，再到 Transformers 库
- ✅ **动手实践**：100+ 个微实践 + 33 个综合项目
- ✅ **高质量内容**：平均质量评分 93.7/100
- ✅ **完整覆盖**：从基础到前沿/多模态，10 个模块 33 个主题
...
本课程采用**双主线并行**设计，让你在不同模态中体会相同技术的迁移应用：

**主线 1（文本/NLP）：`电商客服智能助理`** — 贯穿 Module 1-9
...
**主线 2（多模态/CV）：`通用多模态平台`**（以**连锁便利店货架监控为贯穿示范场景**）🆕 — M05/M07/M08 含代码实践 + Module 10 综合实战

- M05 实践：LoRA-for-ViT 代码示例（同一技术跨模态验证）→ 指向 Module 10.4 店级 LoRA 微调
- M07 实践：边缘部署 benchmark（PyTorch/ONNX/TensorRT/CoreML 对比）→ 指向 Module 10.4 边缘盒子部署
- M08 实践：CLIP + FAISS 图文混合检索代码 → 指向 Module 10.2 货架以图搜图
- Module 10：连锁便利店货架监控全链路实战（选型→商品分类→视频监控→LoRA微调→边缘部署）
- 两条主线在 M10 末尾设有选做的「双主线合流」项目——把 NLP 主线的电商客服 Agent 接入多模态主线的图像理解能力
  ...
  -> 架构层: RNN/Attention -> Transformer
    -> 训练层: Pre-training -> Fine-tuning -> Advanced Training
      -> 工程层: Deployment -> Applications
        -> 前沿层: Frontiers
          -> 多模态扩展层: Multimodal Small Models (Module 10)
```
...
- 阶段 5（Module 10）：建立多模态与边缘部署能力（扩展分支）
...
📖 [完整模块指南](notebooks/Module01_Foundation/README.md)
...
📖 [完整模块指南](notebooks/Module02_Evolution/README.md)
...
📖 [完整模块指南](notebooks/Module03_Transformer/README.md)
...
📖 [完整模块指南](notebooks/Module04_PreTraining/README.md)

---

### Module 5: 微调技术 (Fine-tuning) 🏆
**3 个 Notebooks | 预计 9-12 小时 | 平均分 93+/100**
...
📖 [完整模块指南](notebooks/Module06_AdvancedTraining/README.md)

---

### Module 7: 部署与优化 (Deployment)
**3 个 Notebooks | 预计 8-11 小时 | 平均分 92/100**
...
📖 [完整模块指南](notebooks/Module07_Deployment/README.md)
...
- 02 - 图像识别实践（分类/检测/检索、数据增强、模型校准）
- 03 - 视频理解与监控（帧采样、TSM 时序建模、多路实时推理管线）
- 04 - 场景微调与边缘部署（LoRA for ViT、INT8 量化、ONNX/TensorRT、FastAPI 推理平台）

**学习目标**：掌握多模态小模型从选型到边缘部署的完整链路

📖 [完整模块指南](notebooks/Module10_Multimodal/README.md)
...
| 知识领域 | 要求程度 | 说明 |
|---------|---------|------|
| Python 编程 | 必须 | 函数、类、列表推导式等基础语法 |
| NumPy 基础 | 必须 | 数组创建、索引、广播机制 |
| 线性代数直觉 | 建议 | 向量、矩阵乘法直觉即可，无需严格证明 |
| 微积分基础 | 建议 | 导数和梯度的概念 |
| 概率论基础 | 建议 | 概率分布、期望的基本概念 |

> 完全不具备以上知识的读者，建议先完成 Module 1 的数学复习部分，再继续后续模块。
...
| 路径 | 目标群体 | 时间 | 核心模块 | 目标 |
|------|---------|------|---------|------|
| 🚀 快速入门 | 初学者 | 40-50 小时 | M1→M2→M3→M4→M5(LoRA) | 理解 LLM 基础并能进行简单微调 |
| 🔧 工程实践 | 后端/工程师 | 60-80 小时 | M1-4(快速)→M5→M6→M7→M8 | 掌握 LLM 工程化和应用开发 |
| 🎓 完整学习 | 研究者 | 100-120 小时 | M1→M2→...→M9（NLP 主线） | 全面掌握 LLM 理论与实践 |
| 📱 应用优先 | 产品/后端 | 45-60 小时 | M1→M3→M5(PEFT)→M7→M8 | 保留必要原理，尽快完成可上线应用 |
| 🎥 多模态扩展 | CV/嵌入式 | 12-15 小时 | M10（推荐先修 M5+M7） | 多模态小模型从选型到边缘部署 |
...
**目标**: 掌握 LLM 工程化和应用开发。可选项：完成后可扩展至 Module 10（多模态小模型边缘部署）
...
**目标**: 全面掌握 LLM 理论与实践（NLP 主线）。可选项：完成 Module 10 扩展至多模态/CV 方向
...
### 路径 5：多模态与边缘部署（推荐 CV/嵌入式工程师）🆕
**时间**: 12-15 小时（独立于路径 1-4，需要 DL 基础）

```
Module 10.1 (多模态选型) → Module 10.2 (图像识别) →
Module 10.3 (视频理解) → Module 10.4 (微调与边缘部署)
```

**目标**: 掌握多模态小模型从选型到边缘部署的完整链路
...
- ✅ **33/33 notebooks** (100% 完成)
- ✅ **100+ 微实践**
- ✅ **33 个综合项目**
- ✅ **平均质量评分**: 93.7/100（M1-M9 已评估，M10 待评估）

### 质量分布（已评估 Notebooks）

> 以下数据基于已完成质量评估的 18 个 Notebooks（Module 4-9）
...
> 详细变更历史请参考 [Git 提交记录](../../commits/main)。

### 近期更新
- ✅ 完成全部 9 个模块 29 个 Notebooks（M1-M9 100% 覆盖）
- ✅ 新增 Module 10 多模态扩展分支（4 个 Notebooks）
- ✅ 完成 M4-M9 质量审计，平均分 93.7/100
- ✅ 为全部模块补全 README 和 LEARNING_GUIDE
- ✅ 修复文档一致性问题
