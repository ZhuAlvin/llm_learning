...
**主线 2（多模态/CV）：`通用多模态平台`** 🆕 — 贯穿 M05/M07/M08 穿插点 + Module 10 综合实战

- M05 穿插：LoRA 同样适用于 ViT/CLIP 的视觉微调 → 指向 Module 10.4
- M07 穿插：量化/ONNX 技术在边缘设备部署视觉模型 → 指向 Module 10.4
- M08 穿插：CLIP 共享向量空间实现以图搜图 → 指向 Module 10.2
- Module 10：安防监控/零售分析/医疗影像的全链路实战（选型→训练→微调→部署）
